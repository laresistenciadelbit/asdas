...
main(true,"<?=$config['fm']?>","",moment(new Date()).format('YYYY-MM-DD'),<?=$config['online_threshold_minutes']?>,"<?=$config['primary_sensor']?>",status_to_show,current_station,sensor_maps);

//creamos un evento para escuchar el día del calendario
$('#calendar').on("change.datetimepicker", ({date, oldDate}) => {
	main(true,"<?=$config['fm']?>","",moment(date).format('YYYY-MM-DD'),<?=$config['online_threshold_minutes']?>,"<?=$config['primary_sensor']?>",status_to_show,current_station,sensor_maps);
});

//creamos un evento para escuchar el mes del calendario
$('#calendar').on("update.datetimepicker", ({change, viewDate }) => {
	main(false,"<?=$config['fm']?>",moment(viewDate).format('YYYY-MM'),"",<?=$config['online_threshold_minutes']?>,"<?=$config['primary_sensor']?>",status_to_show,current_station,sensor_maps);
});
